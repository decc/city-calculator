...
\newcommand{\city}{Birmingham}
\newcommand{\citys}{Birmingham's} 
\title{\city}
\author{James Geddes}
\begin{document}
\maketitle
...
options(xtable.floating = FALSE, xtable.booktabs = TRUE, xtable.include.rownames = FALSE, xtable.format.args = list(big.mark = ","))
as_num <- function(n) {
  I(as.numeric(n))
}

## Initialise analytical code
##
...
##
boroughs.co2 <- ddply(all.co2[all.co2$city == the_city, ], 
                      .(geography_code, geography_name, yr, sector0, sector_code, sector), 
                      summarise, CO2 = sum(CO2))

boroughs.pop <- ddply(all.pop[all.pop$city == the_city, ], 
                      .(geography_code, geography_name, yr), 
                      summarise, pop = sum(pop))

## In boroughs.all, the pop variable should not be summed as it contains duplicates.
## Note: boroughs.all is unitless to avoid problems with xtable.
boroughs.all <- transform(merge(boroughs.co2, boroughs.pop, 
                                by = c("geography_code", "geography_name", "yr")),
                          CO2 = as.numeric(as.Quantity(CO2, "kt")),
                          CO2percapita = as.numeric(as.Quantity(CO2, "kt") / pop))
...
The UK's total greenhouse gas emissions in 2010 (excluding international
aviation and shipping) were \SI{590.2}{Mt.\COO e}, of which \SI[round-mode =
places, round-precision = 1]{%
  \Sexpr{as_num(as.Quantity(with(ukregco2, sum(CO2[yr == 2010])), "Mt"))}%
...
%   ------------------------------------------------        --------------------------
    ...
    \bfseries]{\Sexpr{with(ukregco2, as.numeric(as.Quantity(sum(CO2[yr ==
        2010]), "Mt")))}} \\ \bottomrule
  \end{tabular}
...
In 2010, total emissions from \city\ were \SI[round-mode = places, round-precision = 1]{%
\Sexpr{as_num(as.Quantity(sum(boroughs.co2$CO2[boroughs.co2$yr == 2010]), "Mt"))}%
...
\begin{table}[htbp]
  \centering
<<label = summary_all, results = "asis">>=
xtable(dcast(cities.co2, city ~ yr, function(x) {0.001 * sum(x)}, value.var =
             "CO2"), digits = 1)
@ 
...
\Sexpr{as_num(as.Quantity(with(boroughs.co2, sum(CO2[yr == 2010])), "kt") / with(boroughs.pop, sum(pop[yr == 2010])))}%
}{t.\COO}in 2010, which compares with a UK average of \SI[round-mode = places, round-precision = 1]{%
\Sexpr{as_num(as.Quantity(sum(ukregco2$CO2), "kt") / sum(ukregpop$pop, na.rm = TRUE))}%
...
\begin{table}[htbp]
  \centering
<<label = borough_summary, results = "asis">>=
...
xtable(
  ...
  digits = 1)
@ 
\caption{Summary statistics for each of the \Sexpr{n.boroughs} boroughs that
  make up \city. (Emissions are in~\si{kt.\COO}; population figures are mid-year
  estimates in thousands; and emissions per capita are in
  tonnes.)\label{table:borough-summary}}
\end{table}

\begin{table}[htbp]
  \centering\small
<<label = city_co2, results = "asis">>=
xtable(dcast(subset(boroughs.all, yr == 2010),
             geography_name ~ sector0, 
             sum, value.var = "CO2percapita", margins = "sector0"), 
       digits = 1)
@ 
\caption{Per capita emissions from the individual boroughs in 2010. Note that
  the high-level sectors shown here are not identical to those in the data as
  published by DECC as Railways have been included within
...
\begin{table}[htbp]
  \centering\small
<<label = city_all, results = "asis">>=
xtable(
  dcast(co2, sector ~ yr, value.var = "CO2"), digits = 0)
@
...
As at the time of writing, not all energy data was available for 2010, so
table~\ref{table:energy-summary} shows energy consumption for~2009.  
\begin{table}[htbp]
  \centering\small
<<label = energy_summary, results = "asis">>=
energy <- extract_city(the_city, ukregenergy)
energy <- ddply(energy, .(yr, sector, sector0, fuel), summarise, energy = sum(energy))
print(xtable(dcast(subset(energy, yr == 2009), 
                   fuel ~ sector0, sum, value.var = "energy", margins = TRUE), 
             digits = 0))
@ 
\caption{2009 energy consumption for \city, \si{GW.h}.\label{table:energy-summary}}
\end{table}

Given the energy data and a set of emissions factors, we can compute the
emissions implied by the energy consumption. Table~\ref{table:impliedemissions}
shows the result using emissions factors corresponding to direct emissions
only (apart from electricity). The computed emissions are lower than those in
the regional statistics, because those include certain indirect emissions;
notably those from extraction and refining of fuels. 

\begin{table}[htbp]
  \centering\small
<<label = emissions_from_energy, results = "asis">>=
ef <- c(fuel.co2, electricity = electricity.co2)
energy$ef <- ef[match(energy$fuel, names(ef))] 
energy$co2 <- as.Quantity(energy$energy * energy$ef, "kt")
print(xtable(dcast(subset(energy, yr == 2009), 
                   fuel ~ sector0, sum, value.var = "co2", margins = TRUE), 
             digits = 0))
@ 
\caption{2009 computed emissions, \si{kt.\COO}.\label{table:impliedemissions}}
\end{table}

Table~\ref{table:comparison} shows that the difference is less than
\SI{3}{\percent} in total and most of that comes from the transport sector, where
the implied emissions underreport by about~\SI{10}{\percent}.

\begin{table}[htbp]
  \centering
<<label = comparison, results = "asis">>=
computed <- ddply(subset(energy, yr == 2009),
                  .(sector0), summarise, computed = as.numeric(as.Quantity(sum(co2), "Mt")))
regional <- ddply(subset(co2, yr == 2009),
                  .(sector0), summarise, regional = as.numeric(as.Quantity(sum(CO2), "Mt")))
comp <- merge(computed, regional, all = TRUE)
comp$sector0 <- as.character(comp$sector0)
comp <- rbind(comp,
              data.frame(sector0 = "(all)", 
                         computed = sum(comp$computed, na.rm = TRUE), 
                         regional = sum(comp$regional, na.rm = TRUE)))
xtable(comp, digits = 2)
@ 
\caption{Computed emissions and emissions from the regional emissions
  statistics, high level sectors, 2009, in~\si{Mt}.\label{table:comparison}}
\end{table}
\printbibliography

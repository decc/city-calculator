...
## Summaries by year at borough level for this city 
##
boroughs.co2 <- ddply(all.co2[all.co2$city == the_city, ],
                      .(geography_code, geography_name, yr), summarise, CO2 = sum(CO2))

boroughs.pop <- ddply(all.pop[all.pop$city == the_city, ],
                      .(geography_code, geography_name, yr), summarise, pop = sum(pop))

boroughs.all <- merge(boroughs.co2, boroughs.pop, by = c("geography_code", "geography_name", "yr"))

boroughs.all$CO2percapita <- with(boroughs.all, CO2 / pop)

## Emissions per capita by sector0 at borough level
##
...
\end{document}
%%% TEMPORARY END OF DOCUMENT FOR DEBUGGING

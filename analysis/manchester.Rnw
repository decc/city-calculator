...
the_city = "Manchester"
## Add names of cities and high-level sectors to UK regional statistics
cities.co2 <- subset(merge(merge(cities, ukregco2), sector0),
                     select = c("city", "la_name", "sector0", "sector_name", "yr", "CO2"))

cities.pop <- subset(merge(cities, ukregpop),
                     select = c("city", "la_name", "yr", "pop"))
## Summarise emissions and population by year for the city.
the_city.co2 <- ddply(cities.co2[cities.co2$city == the_city, ],
                      .(yr), summarise, CO2 = sum(CO2))
the_city.pop <- cities.pop[cities.pop$city == the_city, ]
@ 
...
\section{Overview of \citys\  emissions}
...
1]{%
\Sexpr{0.001 * sum(the_city.co2$CO2[the_city.co2$yr == 2010])}%
...
\end{table}
%
Emissions per capita in \city\ were
\SI[round-mode = places, round-precision = 1]{%
\Sexpr{with(the_city.co2, sum(CO2[yr == 2010])) / with(the_city.pop, sum(pop[yr == 2010]))}%
...
city <- cities.co2[cities.co2$city == the_city, ]
city0 <- ddply(city, .(la_name, yr), summarise, CO2 = sum(CO2))
city0 <- transform(merge(city0, ukregpop), CO2percapita = CO2 / pop)
n.boroughs <- length(cities$city[cities$city == the_city])
format(subset(city0, 
              yr == 2010, 
              select = c("la_name", "CO2", "pop", "CO2percapita")),
       big.mark =",", digits = 2)
@ 
...
cities.sector0.co2 <- ddply(cities.co2, .(city, la_name, sector0, yr), 
                            summarise, CO2 = sum(CO2))
cities.sector0.co2 <- merge(cities.sector0.co2, cities.pop)
cities.sector0.co2 <- transform(cities.sector0.co2, CO2percapita = CO2 / pop)
format(dcast(subset(cities.sector0.co2, city == the_city & yr == 2010),
             la_name ~ sector0, 
             sum, value.var = "CO2percapita", margins = "sector0"), 
       big.mark = ",", digits = 2)
@ 
\caption{Per capita emissions from the individual boroughs in 2010. The abbreviated
  sectors are ``Industrial and Commercial'' and ``Large Industrial
  Installations''. Note that the high-level sectors shown here are not identical
  to those in the data as published by DECC as Railways have been included within
...
levels~\cite{mcc-future}. So, for the purpose of this document, we will assume
that the 2020 target is a reduction of about \SI{40}{\percent} against 2005
levels.
